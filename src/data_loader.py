...
def _synthetic_price_series(symbol: str, start="2018-01-01", periods=1650, seed=7):
    ...
    df = pd.DataFrame({"date": dates, "ticker": symbol.upper(), "close": close, "volume": volume})
    return df

def load_vnindex():
    # Try vnstock first; fallback to synthetic data.
    try:
        from vnstock import Vnstock
        stock = Vnstock().stock(symbol="VNINDEX", source="VCI")
        df = stock.quote.history(start="2018-01-01", end=pd.Timestamp.today().strftime("%Y-%m-%d"), interval="1D")
        df = df.rename(columns={"time": "date"})
        if "date" not in df.columns:
            df = df.reset_index().rename(columns={"index": "date"})
        df["date"] = pd.to_datetime(df["date"])
        df["ticker"] = "VNINDEX"
        df = df[["date", "ticker", "close", "volume"]].dropna()
        if len(df) > 200:
            return df
    except Exception:
        pass
    ...
    try:
        ...
        for t in tickers:
            try:
                ...
                df = stock.quote.history(start="2021-01-01", end=pd.Timestamp.today().strftime("%Y-%m-%d"), interval="1D")
                df = df.rename(columns={"time": "date"})
                if "date" not in df.columns:
                    df = df.reset_index().rename(columns={"index": "date"})
                df["date"] = pd.to_datetime(df["date"])
                ...
                if "volume" not in df.columns:
                    df["volume"] = np.nan
                frames.append(df[["date", "ticker", "close", "volume"]].dropna(subset=["date", "close"]))
            except Exception:
                frames.append(_synthetic_price_series(t, start="2021-01-01", periods=900))
    except Exception:
        frames = [_synthetic_price_series(t, start="2021-01-01", periods=900) for t in tickers]
